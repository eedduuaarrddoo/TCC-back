...
import { Table2, Pencil, Trash2, Search } from "lucide-react";
import { getAllMetodologias, searchMetodologiasByNome, getSamplesByMetodologia } from "../controllers/sampleController";
...
const MetodologiaList: React.FC<Props> = ({ onEdit, onView, onDelete }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      if (searchNome.trim() === "") {
        const all = await getAllMetodologias();
        setMetodologias(all);
      } else {
        const results = await searchMetodologiasByNome(searchNome.trim());
        setMetodologias(results);
      }
    } catch (err) {
      console.error("Erro ao buscar metodologias:", err);
      setError("Erro ao buscar metodologias.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGeneratePdf = async (metodologia: MetodologiaData) => {
    try {
      ...
      if (discoSamples.length > 0) {
        ...
        doc.text(`DiscoSamples da Metodologia: ${metodologia.nome}`, 14, nextY - 2);
  ...
  return (
    <div className="metodologia-list user-panel">
